...
async function run() {
  ...
  const schemaSQL = `
    ...
    DO $$ BEGIN
      CREATE TYPE tender_source AS ENUM ('ted_europa', 'sam_gov', 'who_procurement', 'un_agencies', 'nhs_supply_chain', 'contracts_finder', 'find_a_tender', 'manual');
    ...
    DO $$ BEGIN
      CREATE TYPE procurement_category AS ENUM (
        ...
        'surgical_instruments', 'telemedicine', 'clinical_services', 'allied_health', 'occupational_therapy', 'paramedic_services', 'patient_transport', 'social_care', 'menstrual_health', 'wash_hygiene', 'other'
      );
    EXCEPTION WHEN duplicate_object THEN null; END $$;
