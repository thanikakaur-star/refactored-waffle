...
function getClient() {
    if (!_client)
        _client = new sdk_1.default({ apiKey: config_js_1.config.anthropic.apiKey });
    return _client;
}
...
async function chat(history, userMessage) {
    ...
    logger_js_1.logger.info("Chatbot request", { messageCount: messages.length });
    ...
    const text = response.content
        ...
        .join("");
    logger_js_1.logger.info("Chatbot response generated", { tokens: response.usage.output_tokens });
    return text;
}
async function* chatStream(history, userMessage) {
    const messages = [
        ...history,
        { role: "user", content: userMessage },
    ];
